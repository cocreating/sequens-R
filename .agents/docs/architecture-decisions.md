...
- Versioned example documents may ship under `public/projects/` and enter the app only through the ordinary import/migration boundary. A validated `public/projects/index.json` catalog drives the Workspace demo picker. The catalog contains exactly fifteen Piano-forward schema-version-5 ambient, classical-form, and sweet-electronic demos at 56–126 BPM. Each has one to three musical modules, eight slots per module, expressive Piano patterns, and Prelude/Theme/Variation/Finale scenes. Demos persist rack mix state and use the permanent Mixer view without adding a Mixer module. The two Basic Electro documents remain unlisted migration fixtures rather than catalog entries. All catalog files are regenerated with `npm run demos:generate`.
