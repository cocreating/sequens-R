...
- Versioned example documents may ship under `public/projects/` and enter the app only through the ordinary import/migration boundary. A validated `public/projects/index.json` catalog drives the Workspace demo picker. Each entry carries a required genre, and the current fifteen Piano-forward schema-version-5 demos are grouped as five Neoclassical Ambient, five Post-Classical Minimalism, and five Melodic Electronica projects at 56–126 BPM. Each has one to three musical modules, eight slots per module, expressive Piano patterns, and Prelude/Theme/Variation/Finale scenes. Demos persist rack mix state and use the permanent Mixer view without adding a Mixer module. The two Basic Electro documents remain unlisted migration fixtures rather than catalog entries. All catalog files are regenerated with `npm run demos:generate`.
...
- Tap BPM is an explicit post-SDD transport refinement. A native header button averages recent valid tap intervals and writes an integer from 20–300 BPM; manual edits also write integers. The former increment/decrement pair has been replaced by a native vertical 20–300 range input disclosed from the tempo field on pointer hover or focus, preserving keyboard and touch access without spending permanent horizontal space. Existing project/share decoders retain tenth-BPM compatibility so older saved patches remain readable.
- The application header, rather than the performance deck, owns sticky positioning. Header actions may wrap; Random and Add Module live beside the other primary actions, while TAP and Play/Pause are last and form the dedicated second mobile row. Add Module opens a native top-layer catalog of icon, name, and description buttons instead of maintaining a separate selected module type. The measured header height positions fixed studio popovers without overlap. Tempo, Root, and Scale stay in a single compact row below. A scroll-position-aware up-arrow is rendered only after 240 CSS px and honors reduced-motion preferences when returning to the document start.
- Project persistence, rack management, scenes, hardware, audio output, exports, shortcuts, and diagnostics remain fully available inside one top-layer `Workspace` panel. An icon-only toolbox button immediately after TAP invokes the native popover; its accessible name remains `Workspace`.
- The shared rack mixer is a permanent application surface opened from the header immediately beside Workspace. It uses a full-width top-layer panel and does not require a persisted Mixer module; older projects and duplicate Mixer module views remain supported. Its vertical faders write the existing module level state, while the segmented LED ladders read existing diagnostic peaks, so this presentation change introduces no project, share, scheduler, or audio-graph schema changes.
...
- The final audible catalog contains 12 Acid presets, 40 non-Acid presets (Bass 8, Chords 10, Arp 8, Piano 8, Euclid 6), and six Drum kits, plus explicit silent Mixer/CC/Mod records. Patch schema 4 freezes the compact order of this released v2-only registry.
...
- Future changes for all ten modules are centralized in `module-improvement-roadmap.md`. That roadmap may propose work but cannot silently override accepted generator, MIDI, persistence, audio, performance, or accessibility contracts.
