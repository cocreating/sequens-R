...
- Versioned example documents may ship under `public/projects/` and enter the app only through the ordinary import/migration boundary. A validated `public/projects/index.json` catalog drives the Workspace demo picker. Each entry carries a required genre, and the current twenty schema-version-7 demos are grouped as seven Minimal Techno, seven Minimal House Techno, and six Ambient Techno & Breakbeats projects at 84–168 BPM. Every demo uses two or three musical modules with eight slots each and exposes Intro/Groove/Variation/Peak scenes drawn from slots 2, 0, 5, and 3 so the four scenes form one arc. Synth is not mandatory: a demo may omit it where the archetype has no lead voice. A demo declares at most four of the eight drum lanes, and every pattern slot is masked to that set, so the four-sound ceiling holds in all four scenes. Demos persist rack mix state and use the permanent Mixer panel. The original Basic Electro document remains an unlisted migration fixture rather than a catalog entry. All catalog files are regenerated with `npm run demos:generate`.
...
- **Module plates do not clip.** `article` uses `overflow: visible` so the per-module actions menu, which is absolutely positioned inside the plate, is not cut off when the module is collapsed and the plate is only one row tall. The spine and progress bar carry their own radii in place of the former clip, and a plate with an open menu is raised to `z-index: 8` because each plate is its own stacking context via `view-transition-name`. The clip that actually cut the menu was not `overflow` but `content-visibility: auto` on `.module-list > article`, which implies paint containment; it is lifted while the menu is open.
- **The actions menu dismisses like a menu.** Once the plate stopped clipping, an open `details` menu genuinely painted over the module below and swallowed its clicks, which the clipping had hidden. It now closes on an outside pointer press and on Escape, restoring focus to its summary, and one-shot items — duplicate, export MIDI, delete, colour change — close it on selection. Move earlier/later deliberately leave it open: reordering is repeatable and the menu travels with its own module.
- **Opening a module's actions menu expands a collapsed module.** Adopted on 2026-09-04 at the user's explicit request, superseding the initial rejection. `collapsed` is rack state persisted in the project document and routed through the ordinary patch path, so this makes opening a menu a real, undoable project edit that autosave will persist — the trade accepted in exchange for the menu never opening against a one-row plate. The module is not re-collapsed on close.
