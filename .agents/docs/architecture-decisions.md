...
- The former performance deck is renamed `header-initial-core-actions` and groups visible `TAB` followed by icon-only Tempo, Root, and Scale inside `transport-fields`. Tempo, Root, and Scale use native top-layer panels; direct option-button grids replace the two selects while dynamic accessible names expose the current BPM, root, and scale from the compact triggers. At the desktop media boundary that group renders inside the sticky application header before all primary header actions; below it, the same component renders once in document flow. This responsive component relocation prevents duplicate IDs and preserves DOM/focus order. Random and Add Module live beside the other primary actions, and Add Module opens a native top-layer catalog of icon, name, and description buttons instead of maintaining a separate selected module type. The measured header height positions fixed studio popovers without overlap. A scroll-position-aware up-arrow is rendered only after 240 CSS px and honors reduced-motion preferences when returning to the document start.
