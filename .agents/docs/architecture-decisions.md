...
- The application header, rather than the performance deck, owns sticky positioning. Header actions may wrap, Add Module lives beside the other primary actions, and the measured header height positions fixed studio popovers without overlap. Tempo, Root, and Scale stay in a single compact row below. A scroll-position-aware up-arrow is rendered only after 240 CSS px and honors reduced-motion preferences when returning to the document start.
