...
On 2026-08-28, the former ten-genre-plus-two-legacy picker was replaced by exactly fifteen schema-version-5 projects generated by `npm run demos:generate`. The new catalog emphasizes original ambient, minimal, and classical-form material, with restrained sweet-electronic hybrids and a hard 130 BPM ceiling. Every project features the upgraded Piano patterns with authored velocity and accents; module counts range from one to three, every module has eight slots, and the four scenes are Prelude, Theme, Variation, and Finale. The shared rack mix remains exposed by the permanent Mixer view and no catalog demo contains a Mixer module. The Basic Electro documents remain unlisted migration fixtures. Production import/build checks and deterministic regeneration pass. Human listening approval for the fifteen complete mixes and their scene transitions remains open and is not implied by automated acceptance. See `demo-projects.md` for the complete matrix and maintenance contract.

Post-catalog verification passed with 0 Svelte/TypeScript errors or warnings, 122 unit tests across 18 files, a 115.14 KiB gzip initial-JavaScript build against the 200 KiB limit, and all 56 Playwright tests. Scheduler-message jitter measured 0.679 ms σ; the five- and fourteen-module browser mixes measured −15.54 LUFS-I / −3.07 dBTP and −13.49 LUFS-I / −1.17 dBTP. These results preserve the automated Phase 7 gates but do not close Piano, complete-mix, transition, or physical-device listening evidence.
