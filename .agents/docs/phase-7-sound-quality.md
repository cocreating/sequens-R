...
Phase 7.11 implementation is complete, but Phase 7 acceptance remains open. Required evidence still missing is explicit Mixer, Piano, Euclid, and final mixed-starter listening approval plus the physical 16-module/140 BPM Android C10 run. The local browser scheduler-message gate was closed by the 2026-08-27 performance remediation below; physical MIDI jitter remains part of C10.

## Piano-forward demo library follow-up

On 2026-08-28, the former ten-genre-plus-two-legacy picker was replaced by exactly fifteen schema-version-5 projects generated by `npm run demos:generate`. The catalog now classifies those projects into three contemporary genres of five—Neoclassical Ambient, Post-Classical Minimalism, and Melodic Electronica—using a validated genre field and accessible grouped picker headings. Every project features the upgraded Piano patterns with authored velocity and accents; module counts range from one to three, every module has eight slots, tempos stay below 130 BPM, and the four scenes are Prelude, Theme, Variation, and Finale. The shared rack mix remains exposed by the permanent Mixer view and no catalog demo contains a Mixer module. The Basic Electro documents remain unlisted migration fixtures. Production import/build checks and deterministic regeneration pass. Human listening approval for the fifteen complete mixes and their scene transitions remains open and is not implied by automated acceptance. See `demo-projects.md` for the complete matrix and maintenance contract.

Post-classification verification passed with 0 Svelte/TypeScript errors or warnings, 123 unit tests across 18 files, a 115.39 KiB gzip initial-JavaScript build against the 200 KiB limit, and all 56 Playwright tests. Scheduler-message jitter measured 2.841 ms σ; the five- and fourteen-module browser mixes measured −15.15 LUFS-I / −3.18 dBTP and −13.39 LUFS-I / −1.59 dBTP. These results preserve the automated Phase 7 gates but do not close Piano, complete-mix, transition, or physical-device listening evidence.
...
## Performance remediation evidence · 2026-08-27

The Android report of rough playback and clipped/distorted sound above three modules triggered a bounded engine pass without changing generator, project, share, MIDI, or SMF contracts:

- adaptive pre-limiter headroom now follows the squared level power of audible modules with events, neutral master character bypasses nonlinear shaping, and the final output trim is −2 dB;
- a coarse-pointer mobile profile uses Chords/Arp/Piano pools of 5/3/4 with a 32-voice rack budget; desktop live playback uses 8/4/8 with a 64-voice budget, and offline rendering keeps full fidelity;
- inactive and empty modules do not retain voice graphs, delay/reverb are lazy, analyser taps follow visible meter surfaces, Acid skips fully silent worklet quanta, and the clock/UI diagnostic rates fall while idle or hidden;
