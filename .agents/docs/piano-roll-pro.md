...
The shipping demo picker now contains fifteen Piano-forward projects generated from fifteen of the key-aware melody examples and visibly classifies them as five Neoclassical Ambient, five Post-Classical Minimalism, and five Melodic Electronica projects. The pool spans 16-, 32-, and 64-step phrases, Piano-only studies through three-module arrangements, varied velocities, accents at the editor's 112 floor, displaced entrances, softer and thinner variants, octave voicing, denser finales, and micro-timing. Tempos stay between 56 and 126 BPM. Each project exposes Theme plus Prelude, Variation, and Finale scene treatments without storing a Mixer module. Structural acceptance and exact five-per-genre grouping are automated in `tests/unit/example-project.test.ts`; listening approval remains open and is tracked in `demo-projects.md` and the Phase 7 evidence.
