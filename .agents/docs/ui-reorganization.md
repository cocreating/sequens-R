...
Status: implementation complete on 2026-08-24; follow-up refinements and current regression acceptance recorded through 2026-09-01.
...
- The rack mixer is the only mixer UI: an always-available full-width top-layer panel opened by the header button beside Workspace. Mixer is absent from New module and retired Mixer modules are not retained or rendered. Channel gain uses keyboard-accessible native vertical faders with hardware-style caps and bottom-up fill; channels and the rack master pair them with live 12-segment green/amber/red dBFS LED ladders.
...
- The floating mixer starts with PAN and SENDS hidden to prioritize channel levels. Two pressed-state toggles in its heading reveal or hide PAN and both send controls across every channel. Narrow mobile portrait targets three channels per row when the available width permits; the same auto-fitting grid expands through four, five, or six columns on wider phones in landscape and desktop surfaces.
- Each channel orders its heading first, then the vertical level area, then optional rotary controls. The LED ladder sits immediately beside the vertical fader, with the meter first and level control second. When revealed, PAN occupies the first rotary row and Delay/Reverb share the second row on desktop; mobile keeps the rotary group vertically compact so more channels fit across the panel.
...
- Schema-driven control language:
  ...
  - vertical native range faders for mixer levels on every viewport.
...
- The former module-type select has been removed. The visibly and programmatically named `Add Module` header action opens a native top-layer dialog containing ten keyboard-focusable module choices, each with an icon, name, and concise purpose statement. Mixer is intentionally absent because its permanent header panel is the sole rack-mix surface. Below 30rem the catalog occupies the full `100vw × 100dvh`, respects safe-area insets, and keeps its heading/Close row sticky. Choosing one adds it immediately, closes the library, and preserves the 16-module limit.
...
The full command is `npm run verify`. The 2026-09-01 regression passed strict Svelte/TypeScript diagnostics with no errors or warnings, 129 unit tests, all 59 Playwright tests, the production PWA build, and the initial-JavaScript gate at 120.12 KiB gzip against 200 KiB. Browser coverage includes the ten-choice module catalog, permanent Mixer, responsive mixer columns, hidden global PAN/SENDS controls, mobile and desktop channel editing, responsive masonry columns, full-width module spanning, project round trips, sharing, sound rendering, and accessibility flows.
