...
The studio UI has been consolidated around the performance path. The global header now keeps Tap BPM before icon-only Play/Pause, Stop, Share, and General Help controls; repeated taps set a smoothed whole-number BPM, while minus/plus controls to the left of the tempo field make one-BPM changes. Pause preserves the exact transport beat and freezes every playhead, Play continues from it, and Stop resets to zero. Tempo, key, icon-only Random, and module creation form the compact command deck below. Mobile reaches the rack immediately and shortens the brand to `s-R`, while desktop keeps the full title and `Local generative MIDI` subtext and places project, rack, scene, hardware, export, and diagnostics tools in a persistent workspace rail. Module plates expose pattern and musical parameters first, move routing and deterministic scheduling into an `Output & advanced` disclosure, and use schema-selected knobs, steppers, segmented controls, switches, selects, and mixer faders instead of a single slider language. Recognizable frequent actions use compact emoticons with explicit accessible names; ambiguous, destructive, recording, mutation, launch, routing, and module-menu commands retain visible text. See the [UI reorganization evidence](.agents/docs/ui-reorganization.md) and [design QA report](design-qa.md).

**Next phase: Phase 6 — Mobile editing parity.** Arp, Euclid, Piano roll, CC Control, and Mod will become addable and editable on mobile through adaptive editors rather than a compressed desktop layout. The rack stays vertical and progressively disclosed; Piano roll receives a dedicated full-screen editor. The audio engine, deterministic generators, scheduler, MIDI timing, project schema, and share formats remain common across both surfaces. Acceptance requires all ten module types at 375 CSS px and a physical Android performance pass with 16 active modules at 140 BPM. See the [Phase 6 specification](.agents/docs/phase-6-mobile-studio.md).
