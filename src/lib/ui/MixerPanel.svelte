...
<div class="mixer-panel" data-help-key="mixer" role="group" aria-label="Module mixer">
  {#each modules.filter((module) => module.type !== 'mixer') as target (target.id)}
    <div class="mixer-channel">
      ...
      <input id={`mix-${target.id}`} data-help-key="mixer-level" type="range" min="0" max="1" step="0.01" value={target.level} aria-valuetext={`${Math.round(target.level * 100)} percent`} oninput={(event) => onpatch(target.id, { level: levelValue(event) })} />
    </div>
  {/each}
</div>
