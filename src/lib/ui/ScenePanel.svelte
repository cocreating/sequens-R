...
<section class="scene-panel" aria-labelledby="scene-heading">
  <div class="scene-heading">
    ...
    <button id="capture-scene-button" type="button" onclick={oncapture}>Capture scene</button>
  </div>
  ...
  {:else}
    <ul class="scene-list">
      {#each scenes as scene, index (scene.id)}
        <li>
          <label for={`${scene.id}-name`}>Scene {index + 1} name</label>
          <input id={`${scene.id}-name`} value={scene.name} oninput={(event) => onrename(scene.id, event.currentTarget.value)} />
          <button type="button" disabled={!appliesToRack(scene)} onclick={() => onlaunch(scene)}>Launch {scene.name}</button>
          <button type="button" class="delete" aria-label={`Delete ${scene.name}`} onclick={() => void removeScene(scene.id)}>×</button>
        </li>
      {/each}
    </ul>
  {/if}
