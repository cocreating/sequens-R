...
describe('Phase 7.0 sound domain and migrations', () => {
  ...
  it('migrates project schemas 1 through 6 to released sounds and round-trips v7 deeply', () => {
    ...
    for (const schemaVersion of [4, 5, 6]) {
      ...
      expect(migrateProject(previous).schemaVersion).toBe(7);
    }
  ...
  it('reads additive v4/v5 links, invalidates retired links, and deeply round-trips v6 sound state', async () => {
    ...
    decompressed[0] = 5;
    const previous = new Uint8Array(await new Response(new Blob([decompressed]).stream().pipeThrough(new CompressionStream('deflate-raw'))).arrayBuffer());
    expect(await deserializeRack(previous)).toEqual(normalizeRack(rack));
    decompressed[0] = 3;
  ...
  it('keeps 200 randomized v6 links at or below 400 bytes', async () => {
